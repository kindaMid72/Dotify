...
function Side_Panel({ activeNote, setActiveNote, activeCategory, setActiveCategory }) {
    ...
    return <>
        <div className=' p-3 w-[250px] flex flex-col items-center border-gray-300 border-r-2 pl-6 [&_*]:mb-1 [&_*]:font-mono [&_*]:font-extrabold [&_*]:cursor-pointer bg-gray-100 [&_li]:p-1 [&_li]:rounded-md [&_li]:border-2 [&_li]:border-transparent [&_li]:transition-color [&_li]:ease-in [&_li]:duration-200 '>
            <button onClick={() => setActiveNote(true)} className='border-2 w-full border-black rounded-md px-2 py-1 bg-blue-950 text-white font-mono font-[900] !mb-5 hover:bg-blue-800 transition-colors ease-in duration-200'>+ New Note</button>
            <ol className=' flex flex-col justify-start w-[200px] items-start gap-2 [&_i]:mr-2 overflow-auto'>
                ...
                <li onClick={() => focusTags === 'hidden' ? setFocusTags('block') : setFocusTags('hidden')} ><i className="fa-solid fa-tags"></i>Tags</li>
                <li className={`${focusTags} max-w-full !pl-3  flex`}>
                    {/* Tambahkan kelas untuk memotong teks yang panjang */}
                    <div className="border-x-1 border-black mr-2"></div>
                    <ol className="flex flex-col max-w-fit overflow-hidden [&_li]:overflow-hidden [&_li]:whitespace-nowrap [&_li]:text-ellipsis ">
                        <li className={setActiveClass('tag1')} onClick={() => setActiveCategory('tag1')}>tag1asdfasdfasdfasdfasdf</li>
