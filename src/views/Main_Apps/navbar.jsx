        ...
        <nav id="container" className="p-2 pr-4 bg-gray-200 border-b-[2px] border-gray-300">
            <ol className=" flex justify-end items-center gap-1.5">
                <li className="flex justify-start items-center flex-1 pl-4">
                    ...
                    <p className="font-bold text-xl font-mono pl-2">Dotify</p>
                </li>
                ...
                <li onClick={handleThemeChange} className="border-2 border-black rounded-full bg-gray-100 size-[35px] p-1.5 flex justify-center items-center cursor-pointer">
                    <i className={ themeMode === 'light' ? "fa-solid fa-moon" : "fa-solid fa-sun"}></i>
                </li>
                <li ref={userMenuRef} id="user-profile" className="relative">
                    <button onClick={() => setShowMenu(prev => !prev)} className="border-2 border-black bg-gray-100 rounded-full size-[35px] p-1.5 flex justify-center items-center cursor-pointer">
                        <i className="fa-solid fa-user" ></i>
                    </button>
                    {/* 2. Menu sekarang menjadi anak dari elemen yang memiliki ref */}
                    <div className={showMenu ? `absolute rounded-md w-max bg-gray-300 p-3 top-full mt-2 right-0 z-20` : `hidden`}>
                        <h3 className='pb-3'>{jwt ? JSON.parse(atob(jwt.split('.')[1])).email : 'User'}</h3>
                        <hr className="border-gray-400" />
                        <button onClick={handleLogout} className='pt-2 text-red-700 hover:text-red-400 w-full text-left'>Logout</button>
                    </div>
                </li>
            </ol>
